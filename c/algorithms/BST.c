...
void testCases() {
    ...
    // Test Insert and Search functions
    Insert(&RootPtr, 10);
    assert(Search(RootPtr, 10) == 1); // Expected output: 1

    Insert(&RootPtr, 5); 
    assert(Search(RootPtr, 5) == 1); // Expected output: 1

    Insert(&RootPtr, 15);
    assert(Search(RootPtr, 15) == 1); // Expected output: 1

    Insert(&RootPtr, 20);
    assert(Search(RootPtr, 20) == 1); // Expected output: 1

    assert(Search(RootPtr, 25) == 0); // Expected output: 0
