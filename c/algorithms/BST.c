// Reformatted code with improved comments for c/algorithms/BST.c

#include <stdio.h>
#include <stdlib.h>

// Node structure for the Binary Search Tree
struct BST {
    int data;
    struct BST* left;
    struct BST* right;
};

// Function to create a new BST Node
struct BST *CreateNode() {
    struct BST* newNode = (struct BST*) malloc(sizeof(struct BST));
    newNode->left = NULL;
    newNode->right = NULL;
    return newNode; 
};

// Function to insert a node into the BST
void Insert(struct BST** RootPtr, int value) {
    struct BST* temp = *RootPtr;
    
    // When the list is empty
    if (temp == NULL) {
        struct BST* newNode = CreateNode();
        newNode->data = value;
        *RootPtr = newNode;
    } 
    // If user value is less then current node value insert in left of the node
    else if (value <= temp->data) {
        struct BST* newNode = CreateNode();
        newNode->data = value;
        temp->left = newNode;
    } 
    // If user value is greater then current node value insert at right of the node
    else {
        struct BST* newNode = CreateNode();
        newNode->data = value;
        temp->right = newNode;
    }
}

// Function to search a node in the BST
int Search(struct BST* RootPtr, int item) {
    // Returns 0 if list is empty
    if(RootPtr == NULL) {
        return 0;
    }
    // Returns 1 when element found
    else if(item == RootPtr->data) {
        return 1; 
    }
    // Otherwise search in left side of binary tree if searching value is less then the current node value
    else if(item < RootPtr->data) {
        return Search(RootPtr->left, item); 
    } 
    // Otherwise search in right side of binary tree if searching value is greater then the current node value
    else {
        return Search(RootPtr->right, item); 
    }
}

// Main function
void main() {
    struct BST* RootPtr = NULL;
    int item, cont, key;
    do {
        printf("Enter item: ");
        scanf("%d",&item);
        Insert(&RootPtr, item);

        printf("\n1 to keep inserting/ 0 to Exit: ");
        scanf("%d",&cont);
    } while(cont == 1);

    printf("\nEnter element to search: ");
    scanf("%d",&key);

    if(Search(RootPtr, key) == 1) {
        printf("\nFound\n");
    } else {
        printf("\nNot Found\n");
    }
}