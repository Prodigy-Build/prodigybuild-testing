...
class TestEquatedMonthlyInstallments(unittest.TestCase):
    def test_equated_monthly_installments(self):
        self.assertEqual(
            equated_monthly_installments(25000, 0.12, 3), 830.3577453212793
        )
        self.assertEqual(
            equated_monthly_installments(25000, 0.12, 10), 358.67737100646826
        )
        with self.assertRaises(Exception) as context:
            equated_monthly_installments(0, 0.12, 3)
        self.assertTrue("Principal borrowed must be > 0" in str(context.exception))
        with self.assertRaises(Exception) as context:
            equated_monthly_installments(25000, -1, 3)
        self.assertTrue("Rate of interest must be >= 0" in str(context.exception))
        with self.assertRaises(Exception) as context:
            equated_monthly_installments(25000, 0.12, 0)
        self.assertTrue("Years to repay must be an integer > 0" in str(context.exception))
