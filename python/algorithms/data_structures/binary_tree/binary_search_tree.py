...
import unittest
from algorithms.data_structures.binary_tree.binary_search_tree import BinarySearchTree, Node
...
class BinarySearchTreeTestCase(unittest.TestCase):
    ...
    def test_traversal_tree(self):
        self.assertEqual(
            [node.value for node in self.tree.traversal_tree()],
            [8, 3, 1, 6, 4, 7, 10, 14, 13]
        )

    def test_find_kth_smallest(self):
        self.assertEqual(self.tree.find_kth_smallest(3, self.tree.root), 3)
        self.assertEqual(self.tree.find_kth_smallest(5, self.tree.root), 6)
