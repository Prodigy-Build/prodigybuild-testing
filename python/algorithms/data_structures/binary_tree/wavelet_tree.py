...
def build_tree(arr: List[int]) -> Union[Node, None]:
    root = Node(len(arr))
...
def rank_till_index(node: Union[Node, None], num: int, index: int) -> int:
...
def rank(node: Union[Node, None], num: int, start: int, end: int) -> int:
    if start > end:
        return 0
...
def quantile(node: Union[Node, None], index: int, start: int, end: int) -> int:
...
def range_counting(
    node: Union[Node, None], start: int, end: int, start_num: int, end_num: int
) -> int:
    if (
        start > end
        or node is None
