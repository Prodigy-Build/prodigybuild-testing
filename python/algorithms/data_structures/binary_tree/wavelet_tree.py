...
def rank_till_index(node: Optional[Node], num: int, index: int) -> int:
...
def rank(node: Optional[Node], num: int, start: int, end: int) -> int:
    if start > end:
        return 0
...
def quantile(node: Optional[Node], index: int, start: int, end: int) -> int:
...
def range_counting(
    node: Optional[Node], start: int, end: int, start_num: int, end_num: int
) -> int:
    if (
        start > end
        or node is None
