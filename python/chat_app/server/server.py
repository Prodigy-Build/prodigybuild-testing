...
# Unit test cases
class ChatAppServerTest(unittest.TestCase):
    def setUp(self):
        self.server = ServerWindow()

    def test_get_just_name(self):
        client = "client1"
        self.server.client_map = {"client1": ("address1", "name1", "pubkey1")}
        expected = "name1"
    ...
    def test_join_room_existing_room(self):
        sock = "sock1"
        data = "-join existing_room"
        self.server.rooms = {"existing_room": []}
        self.server.room_map = {"existing_room": []}
        self.server.client_map = {"sock1": ("address1", "name1", "pubkey1")}
        self.server.get_just_name = lambda x: "name1"
        self.server.get_address = lambda x: ("address1", 1234)
        expected_rooms = {"existing_room": ["name1"]}
        expected_room_map = {"existing_room": [UserMap("sock1", "name1")]}
        
        self.server.join_room(sock, data)
        
        self.assertEqual(self.server.rooms, expected_rooms)
        self.assertEqual(self.server.room_map, expected_room_map)
        
    def test_join_room_non_existing_room(self):
        sock = "sock1"
        data = "-join non_existing_room"
        self.server.rooms = {}
        self.server.client_map = {"sock1": ("address1", "name1", "pubkey1")}
        self.server.get_just_name = lambda x: "name1"
        self.server.get_address = lambda x: ("address1", 1234)
        expected = "No such that room !!!"
        
        result = self.server.join_room(sock, data)
    ...
    def test_leave_room(self):
        ...
        user_name = "name1"
        room = "room1"
        self.server.rooms = {"room1": ["name1"]}
        self.server.room_map = {"room1": [UserMap("sock1", "name1")]}
        self.server.client_map = {"sock1": ("address1", "name1", "pubkey1")}
        self.server.get_address = lambda x: ("address1", 1234)
        expected_rooms = {"room1": []}
        expected_room_map = {"room1": []}
        
        self.server.leave_room(sock, user_name, room)
    ...
    def test_create_room_existing_room(self):
        sock = "sock1"
        data = "-create existing_room"
        self.server.rooms = {"existing_room": []}
        self.server.client_map = {"sock1": ("address1", "name1", "pubkey1")}
        expected = "That room is avaiable,you can join in."
        
        result = self.server.create_room(sock, data)
        
        self.assertEqual(result, expected)
        
    def test_create_room_non_existing_room(self):
        sock = "sock1"
        data = "-create non_existing_room"
        self.server.rooms = {}
        self.server.client_map = {"sock1": ("address1", "name1", "pubkey1")}
        self.server.get_just_name = lambda x: "name1"
        self.server.get_address = lambda x: ("address1", 1234)
        expected_rooms = {"non_existing_room": ["name1"]}
        expected_room_map = {"non_existing_room": [UserMap("sock1", "name1")]}
