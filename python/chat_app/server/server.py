...
def test_get_just_name():
    ...
    sock = UserMap(client, "user1")
    assert window.get_just_name(sock) == "user1"

def test_get_address():
    client = socket.socket()
    addr = ("127.0.0.1", 1000)
    sock = UserMap(client, addr)
    assert window.get_address(sock) == addr
...
def test_send_encrypted():
    ...
    message = "Hello"
    name = "user1"
    window.send_encrypted(sock, message, name)
    # No assertion, as a successful send doesn't return anything

def test_updateTableWidget():
    currentRow = window.tableWidget.rowCount()
    assert currentRow == 0
    window.updateTableWidget(True, "user1", "127.0.0.1", 1000)
    currentRow = window.tableWidget.rowCount()
    assert currentRow == 1
    window.updateTableWidget(True, "user2", "127.0.0.1", 2000)
    currentRow = window.tableWidget.rowCount()
    assert currentRow == 2
    window.updateTableWidget(False, "user2", "127.0.0.1", 2000)
    currentRow = window.tableWidget.rowCount()
    assert currentRow == 1
...
def test_join_room():
    sock = socket.socket()
    data = "-join room1"
    window.join_room(sock, data)
    assert window.rooms == {"room1": ["user1"]}
    assert window.room_map == {"room1": [UserMap(sock, "user1")]}

def test_leave_room():
    sock = socket.socket()
    window.rooms = {"room1": ["user1", "user2"]}
    window.room_map = {"room1": [UserMap(sock, "user1"), UserMap(sock, "user2")]}
    window.leave_room(sock, "user1", "room1")
    assert window.rooms == {"room1": ["user2"]}
    assert window.room_map == {"room1": [UserMap(sock, "user2")]}

def test_create_room():
    sock = socket.socket()
    data = "-create room1"
    window.create_room(sock, data)
    assert window.rooms == {"room1": ["user1"]}
    assert window.room_map == {"room1": [UserMap(sock, "user1")]}

def test_setupServer(monkeypatch):
    monkeypatch.setattr(window, "server", "mock_server")
    monkeypatch.setattr(window, "server_privkey", "mock_privkey")
    monkeypatch.setattr(window, "server_pubkey", "mock_pubkey")
    monkeypatch.setattr(window.textEdit, "append", "mock_append")
    window.setupServer()
    assert window.thread_event == threading.Event()
    assert window.c_thread == threading.Thread(target=window.handle_socket, args=(window.thread_event,))

def test_handle_socket(monkeypatch):
    monkeypatch.setattr(window, "socket_list", ["mock_socket"])
    monkeypatch.setattr(window, "outputs", ["mock_output"])
    monkeypatch.setattr(window, "rooms", {"room1": ["user1"]})
    monkeypatch.setattr(window, "room_map", {"room1": ["mock_user"]})
    monkeypatch.setattr(window, "client_map", {"mock_client": ("127.0.0.1", "user1", "mock_pubkey")})
    monkeypatch.setattr(window.textEdit, "append", "mock_append")
    monkeypatch.setattr(window, "updateTableWidget", "mock_updateTableWidget")
    sock = socket.socket()
    data = "test message"
    window.handle_socket(threading.Event())

def test_close_server(monkeypatch):
    monkeypatch.setattr(window.thread_event, "set", "mock_set")
    monkeypatch.setattr(window.server, "shutdown", "mock_shutdown")
    monkeypatch.setattr(window.server, "close", "mock_close")
    window.close_server()