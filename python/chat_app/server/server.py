...
def get_just_name(client):
    return server.client_map[client][1]

def get_address(client):
    return server.client_map[client][0]

def send_encrypted(to_who, message, name):
    try:
        message = message.encode()
        encryptor = server.client_map[to_who][2]
...
class ServerWindow(QMainWindow):
    ...
    def join_room(self,sock,data):
        ...
        user_name = get_just_name(sock)
        host = get_address(sock)
        ...
        if room_name in self.rooms.keys():
            ...
            for o in server.client_map.keys(): send(o,self.rooms)
                            ...
                            if self.rooms == {}:
                                ...
                                if not flag:
                                    ...
                                    if data.split()[0] == "-leave":
                                        ...
                                        for o in self.room_map[room_name]:
                                            if o.sock != sock:
                                                send_encrypted(o.sock,text,user_name)

                        else:
                            self.outputs.remove(sock)
                            address = get_address(sock)
                            user_name = get_just_name(sock)
