...
class ServerWindow(QMainWindow):
    ...
    def setupServer(self):
        ...
        def on_key_generation_completed():
        ...
        Thread(target=generate_rsa_keys).start()
        Thread(target=on_key_generation_completed).start()
        

    def handle_socket(self,thread_event):
        while self.STATE and not thread_event.isSet():

            ready_input,ready_output,error = select.select(self.socket_list,self.outputs,[],3)
